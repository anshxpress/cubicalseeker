    ...
    return( <div className=" px-5 py-8 items-center !text-mine-shaft-100 flex">
        ...
        </div>
        {
            searchFields.map((item,index)=> {
                return <React.Fragment key={index}><div key={index} className="w-1/5"> <Multiinput title={item.title} icon={item.icon} options={item.options}/>
        </div>
        ...
        <RangeSlider onChangeEnd={(e)=>handleChange("exp",e)} color="blue.4" min={1} max={50} minRange={1} size="xs" value={value} labelTransitionProps={{
