...
const TalentCard=(props:any) =>{
    ...
    const handleOffer=(status:string)=>{
        let interview:any= {id, applicantId:profile?. id, applicantStatus:status, interviewTime:date};
        ...
        changeAppStatus(interview).then((res)=>{
           if(status=="INTERVIEWING") successMessage("Interview Schedule", "Interview schedule Successfully");
           else if (status=="OFFERED") successMessage("offered", "Offer had been Accepted");
        ...
        <div className="flex justify-between">
            <div className="flex gap-2 items-center">
                <div className="p-2 bg-mine-shaft-800 rounded-full">
                    <Avatar className="h-7" size="lg"src={profile.picture?`data:image/jpeg;base64,${profile.picture}`:'/Avatar.png'}alt="-ml-2"/>
                </div>
                <div>
                    <div className="font-semibold size=lg">
                        {props.name}
                    </div>
                    <div className="text-xs text-mine-shaft-300">{profile.jobTitle}
    ...
    <div className="flex gap-2 [&>div]:py-1 [&>div]:px-2 [&>div]:bg-mine-shaft-800
     [&>div]:text-sky-400 [&>div]:rounded-lg text-xs">
                {
                    profile.skills?.map((skill: any,index: any)=> index < 4 && <div key={index} className="p-2 py1 bg-mine-shaft-800 text-sky-400 rounded-lg text-xs">{skill}</div>)
                }
    </div>
    ...
    <Divider size="xs" color="blue.7"/>
        ...
        }  
        {
            props.invited && <>
            <div>
            <Button color="blue.4" onClick={()=>handleOffer("OFFERED")} variant="outline" fullWidth>Accept</Button>
            </div>
            <div>
            <Button color="blue.4" onClick={()=>handleOffer("REJECTED")} variant="light" fullWidth>reject</Button>
            </div>
            </>
        }  
    </div>
    {
       (props.invited || props.posted)&& <Button color="blue.4" variant="filled" fullWidth onClick={openApp} autoContrast>View Application</Button>
    }
    <Modal opened={opened} onClose={close} title="Schedule Interview">
       ...
       <Button onClick={()=> handleOffer("INTERVIEW")} color="blue.4" variant="light" fullWidth>Schedule</Button>
       </div>
      </Modal>
      <Modal opened={app} onClose={close} title="Application">
        ...
        <div className="flex flex-col gap-4">  
        <div>  
             Email: &emsp;<a className="text-sky-400 hover:underline cursor-pointer text-center" href={`mailto:${props.email}`}>{props.email}</a>  
        </div>  
        <div>  
            Website: &emsp;<a target="_blank" className="text-sky-400 hover:underline cursor-pointer text-center" href={`${props.website}`}>{props.website}</a>  
        </div> 
        <div>  
            Resume: &emsp;<span className="text-sky-400 hover:underline cursor-pointer text-center" onClick={()=>openBase64PDF(props.resume)}>{props.name}</span>  
        </div> 
        <div>  
