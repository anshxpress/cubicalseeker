    ...
    return <div className="w-2/3">
        <div className="relative">
            ...
            <img className="w-48 h-48 rounded-full -bottom-1/3 absolute left-3
             border-mine-shaft-950 border-5 " src={profile.picture?`data:image/jpeg;base64,${profile.picture}`:'/Avatar.png'} alt="" />
           </div>
         ...
         <div className="px-3">
            ...
            <div className="flex flex-wrap gap-2">
                {
                    profile?.skills?.map((skill:any,index:any)=><div key={index} className="bg-sky-300 font-medium bg-opacity-15 rounded-3xl text-sky-400 px-3 py-1">{skill}</div>)
                }
            </div>           
         </div>
         <Divider ms="xs" size="xs" my="xl" color="blue.7"/>
